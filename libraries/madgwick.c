...
void madgwick_init(madgwick_params* p, float samplePeriod, float beta) {
	...
	p->beta = beta;
	memcpy(p->quaternion, (float[]) { 1.0f, 0.0f, 0.0f, 0.0f }, sizeof(p->quaternion));
}
...
void madgwick_update(madgwick_params* p, float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
	...
	float q1 = p->quaternion[0], q2 = p->quaternion[1], q3 = p->quaternion[2], q4 = p->quaternion[3];
	float norm;
	...
	qDot1 = 0.5f * (-q2 * gx - q3 * gy - q4 * gz) - p->beta * s1;
	qDot2 = 0.5f * (q1 * gx + q3 * gz - q4 * gy) - p->beta * s2;
	qDot3 = 0.5f * (q1 * gy - q2 * gz + q4 * gx) - p->beta * s3;
	qDot4 = 0.5f * (q1 * gz + q2 * gy - q3 * gx) - p->beta * s4;

	// Integrate to yield quaternion
	q1 += qDot1 * p->samplePeriod;
	q2 += qDot2 * p->samplePeriod;
	q3 += qDot3 * p->samplePeriod;
	q4 += qDot4 * p->samplePeriod;

	// normalise quaternion
	norm = 1.0f / sqrt(q1 * q1 + q2 * q2 + q3 * q3 + q4 * q4);
	p->quaternion[0] = q1 * norm;
	p->quaternion[1] = q2 * norm;
	p->quaternion[2] = q3 * norm;
	p->quaternion[3] = q4 * norm;
}
