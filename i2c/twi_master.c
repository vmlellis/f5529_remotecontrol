...
void twi_master_init(void)
{
	...
	//UCB1IE |= (UCNACKIE|UCRXIE|UCTXIE);
	//UCB1IE |= (UCALIE|UCNACKIE|UCSTTIE|UCSTPIE|UCRXIE|UCTXIE);
}
...
uint8_t twi_master_writeTo(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
	...
	UCB1CTL1 |= UCTR;           // Configura como modo de trasnmissão
	UCB1I2CSA = address;        // Seta o endeço do dispositivo escravo
	...
	if(sendStop)
	{
		while (UCB1CTL1 & UCTXSTP);	// aguarda finalizar com uma condição stop
	}
	else
	{
		while (UCB1CTL1 & UCTXSTT);	// aguarda finalizar com uma condição (re)start
	}
...
uint8_t twi_master_readFrom(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
    ...
    UCB1CTL1 &= ~(UCTR);        // Configure in receive mode
    UCB1I2CSA = address;        // Seta o endeço do dispositivo escravo
    ...
    if(length == 1) {
        while(UCB1CTL1 & UCTXSTT);	// Aguarda o bit de start ser enviado
        UCB1CTL1 |= UCTXSTP;        // Envia a condição de stop
    }
 ...
 * UCB1TXIFG eh setado quando UCB1TXBUF estah vazio.
 */
void i2c_tx(void) {
	// se existe dados para enviar, envie, senão pare
	if(twi_masterBufferIndex < twi_masterBufferLength){
		// Copia os dados para o registrados de saida.
		UCB1TXBUF = twi_masterBuffer[twi_masterBufferIndex++];
	}else{
		if (twi_sendStop) {
 ...
 * UCB1RXIFG eh setado qando UCB1RXBUF recebe um caracter.
 */
void i2c_rx(void) {
	twi_masterBuffer[twi_masterBufferIndex++] = UCB1RXBUF;
